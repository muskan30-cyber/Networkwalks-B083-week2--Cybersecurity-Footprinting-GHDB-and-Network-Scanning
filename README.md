...
Pentester Name
(Cybersecurity Professional)	Muskan Varshney
...
I have performed these activities only on the systems and devices where I had secured written permission, or on devices/systems that I own myself. All the material in this report is for education and research purposes only. Nothing here should be used to break the law. My instructor, the authors and Networkwalks are not responsible for what anyone does with this knowledge. Every action taken is my own responsibility. Misuse of these techniques can lead to criminal charges, heavy fines, loss of employment and a permanent record. In most countries, unauthorised access is a crime even when nothing is damaged.
2. Introduction
...
4.1 Footprinting & Reconnaissance (networkwalks.com)
...
WHOIS
I used WHOIS to obtain publicly available domain registration information. The lookup confirmed the domain is registered with GoDaddy.com, LLC, was created on 6 November 2019 and is set to expire on 6 November 2027. The authoritative name servers are NS6135.HOSTGATOR.COM and NS6136.HOSTGATOR.COM, and the domain is DNSSEC unsigned.
 
Figure 1: WHOIS lookup for networkwalks.com run from Kali Linux.
WhatWeb
I then used WhatWeb to identify the technologies used by the website. The results identified WordPress 7.1 and WordPress Download Manager 3.3.58, running on an Apache server at IP 192.232.216.135, with a 301 redirect from the plain HTTP URL to the HTTPS version of the site.
 
Figure 2: WhatWeb technology fingerprint of networkwalks.com.
Nslookup
Using Nslookup against Google's public DNS resolver (8.8.8.8), I resolved the domain name to its IP address. The result identified 192.232.216.135, matching the IP found by WhatWeb.
 
Figure 3: Nslookup resolving networkwalks.com.
curl -I
I used curl with the -I option to inspect the HTTP response headers without downloading the page body. This confirmed the server returns HTTP/2 200, is running Apache, and exposes the WordPress REST API endpoint via the link header (/wp-json/), along with a WordPress-specific cache header (x-nginx-cache: WordPress).
 
Figure 4: HTTP response headers for networkwalks.com via curl -I.
Wafw00f
Next, I used Wafw00f to check whether a Web Application Firewall was protecting the website. The result identified ModSecurity (SpiderLabs) as the WAF in front of the site.
 
Figure 5: Wafw00f identifying the ModSecurity (SpiderLabs) WAF.
DNSRecon
Finally, I used DNSRecon to enumerate DNS records. The scan returned the SOA and NS records (ns6135/ns6136.hostgator.com), the BIND version of both name servers (9.16.23-RH), the MX record (mail.networkwalks.com), an SPF/TXT record authorising Hostgator's mail infrastructure, a Google site-verification TXT record, and eight SRV records pointing autodiscover traffic to cpanelemaildiscovery.cpanel.net.
 
Figure 6: DNSRecon enumeration of networkwalks.com DNS records.
...
1	http://99.114.240.169:8080/	intitle:"Webcam" inurl:WebCam.htm
2	https://www.lmc.edu/webcam.htm	intitle:"Webcam" inurl:WebCam.htm
3	https://www.skylinewebcams.com/webcam/italia/lazio/roma/piazza-di-spagna.html	inurl:webcam site:skylinewebcams.com inurl:roma
4	cameraftp.com live camera share (Airport north)	intitle:"Login" intext:"camera"
5	cameraftp.com live camera share (Mt Washington)	intitle:"Login" intext:"camera"
6	cameraftp.com live camera share (Webcam meteoferrals)	intitle:"Login" intext:"camera"
7	cameraftp.com live camera share (Kaeo)	intitle:"Login" intext:"camera"
8	http://109.233.191.130:8080/	intitle:"webcamXP" inurl:8080
9	http://109.206.96.75:8080/	intitle:"webcamXP" inurl:8080
10	http://75.149.26.30:1024/	intitle:"webcamXP" inurl:8080
...
3	https://justmathbg.info/files/	intitle:index.of "parent directory" mathematics pdf
4	https://education.giakonda.org.uk/Maths/?SD	intitle:index.of "parent directory" mathematics pdf
5	http://www.zaco.au/lib/math/text/	intitle:index.of "parent directory" mathematics pdf
6	https://www.easyteacherworksheets.com/pages/pdf/math/	intitle:index.of "parent directory" mathematics pdf
7	https://www.netlib.org/math/docpdf/	intitle:index.of "parent directory" mathematics pdf
8	https://ochicken.net/library/Mathematics/	intitle:index.of "parent directory" mathematics pdf
9	https://www.wvfa.org/pdf/projectLearningTree/	intitle:index.of "parent directory" mathematics pdf
10	https://www.ellerman.org/Davids-Stuff/Maths/	intitle:index.of "parent directory" mathematics pdf
4.3 Network Scanning with Zenmap
For the third module, I downloaded and installed Zenmap (which bundles Npcap) on my Windows PC, then used it to perform network discovery on my own local network.
 
Figure 10: Npcap installed successfully as part of the Zenmap setup.
I used the Windows ipconfig command to identify my local IP address and LAN subnet. My Wi-Fi adapter was assigned 192.168.1.2 on the 192.168.1.0/24 subnet, with a default gateway of 192.168.1.1.
 
Figure 11: ipconfig output showing my local IP address and LAN subnet, with Zenmap open.
I then entered 192.168.1.0/24 as the target in Zenmap and ran a Ping Scan (nmap -sn 192.168.1.0/24) to identify active hosts.
 
Figure 12: Zenmap ping scan output for 192.168.1.0/24.
Task Results
...
After completing the scan, I opened the Topology section in Zenmap, enabled the legend to understand the symbols, and saved the network topology in PDF format.
 
Figure 13: Zenmap Topology view with the legend enabled, showing all 4 hosts around localhost.
 
Figure 14: Saving the topology graphic in PDF format.
 
Figure 15: Final saved network topology graphic - 4 live hosts around localhost.
5. Risk Analysis / Impact
Based on the information collected during the footprinting, GHDB search and network scanning activities, I identified the following potential risks.
...
6	Internet-connected devices exposed via Google dorking	GHDB dorks located 10 live, unauthenticated webcam interfaces indexed by Google	Anyone with the dork can view or, in some cases, control the exposed cameras without authorisation	Critical
7	Sensitive/private files indexed by search engines	An index-of dork located open directories serving downloadable PDF files with no access control	Any file placed in an open, indexed directory becomes publicly retrievable, including files never meant to be shared	Medium
8	Multiple live hosts visible on local network	Zenmap identified 4 live hosts with resolvable MAC addresses on my home network	Unknown or unauthorised devices may potentially be present on a network	Medium
